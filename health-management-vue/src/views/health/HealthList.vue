    ...
    <div style="display: flex;flex-direction: column;align-items: center">
        ...
        <el-card style="width: 80%">
                ...
                <div>
                    <el-select placeholder="请选择学年" clearable v-model="searchData.clauses[3].value"  @change="initData">
                        <el-option v-for="item in checkDataOptions" :key="item.label" :label="item.label" :value="item.label" ></el-option>
                    </el-select>
                </div>
                <div>
                    <el-input placeholder="根据学院筛选" v-model="searchData.clauses[0].value" @blur="initData">
                        <template slot="append"><el-button icon="el-icon-search" @click="initData"></el-button></template>
                    </el-input>
                </div>
                <div>
        ...
        <el-dialog :title="title"
            ...
            <div style="width: 100%;display: flex;align-items: center;flex-direction: column">
                <el-form :model="checkInfoData" :rules="rules" ref="checkInfoData" label-width="25%" style="width: 90%;" label-position="left">
                    <el-row>
                        <el-col :span="8">
                            <el-form-item label="选择学生" prop="userId">
                                <el-select v-model="checkInfoData.userId" :disabled="disable"  :value="checkInfoData.userId" @change="selectUser">
                        ...
                        <el-col :span="8">
                            <el-form-item label="选择学年" prop="checkYear">
                                <el-select v-model="checkInfoData.checkYear" :disabled="disable"  :value="checkInfoData.checkYear" @change="judgeCheckIsExist">
                                    <el-option v-for="item in checkDataOptions" :key="item.label" :label="item.label" :value="item.label"></el-option>
                                </el-select>
                            </el-form-item>
                        </el-col>
        ...
        <el-dialog title="查看学生健康档案"
            ...
            <health-table :health-data="checkInfoData" :is-msg="true" :disabled="disable" @save-suggestion="saveSuggestion"></health-table>
        </el-dialog>
    </div>
</template>
        ...
        methods:{
            ...
            selectUser(){
               let data =  this.userOptions.filter((value) => {
                 console.log("返回的学生id=》》》",value.id)
                   return this.checkInfoData.userId  === value.id;
               });
            ...
            judgeCheckIsExist(){
                if (this.checkInfoData.userId === "" || this.checkInfoData.userId == null){
                    ...
                    this.$http.get("/checkInfo/judgeCheckIsExist",{params:{"userId":this.checkInfoData.userId,"checkYear":this.checkInfoData.checkYear}}).then(res=>{
                        if (res.status === 200){
                            if (res.data===false){
